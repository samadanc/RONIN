...
def start_dynamic_analysis(app_path: str, app_name: str):
    ...
    for app in app_versions:
        ...
        uber_signer_path = "C:\\Users\\samad\\Projects\\RONIN\\dynamic_analysis\\utils\\uber-apk-signer-1.2.1.jar"
        # app = "C:\\Users\\samad\\Projects\\RONIN\\SOOTOUT\\BlockCipher-ECB-InformationExposure-Lean-benign\\BlockCipher-ECB-InformationExposure-Lean-benign0\\BlockCipher-ECB-InformationExposure-Lean-benign.apk"
        java_command = ['java', '-jar', uber_signer_path, "-a", app, "--overwrite", "--allowResign"]#sh_split(f'java -jar {uber_signer_path} -a {app} --overwrite --allowResign')
        subprocess.call(java_command, timeout=10000000000) #TODO: set timer back later
        # loading vulnerabilities
...
def close_old_appium_services():
    print("passing on closing old appium processes")
